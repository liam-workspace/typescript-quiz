...
adopting them would make the design *worse*, not merely redundant — each says
...
NestJS's global `ValidationPipe` threw `BadRequestException` *before any
controller ran* — no handler, no log, no row. Choosing NestJS inherits that
...
offline app-shell is what makes that true of the *page*, not just of the
